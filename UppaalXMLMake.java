...
public class UppaalXMLMake extends UppaalXML{
    ...
    Location[] LocationMake(Function.Func[] funclist){
        ...
        for(int i=0;i<funclist.length;i++){
            if(funclist[i].type == Function.FuncType.SSF){
                loc = locSet(ssfnum*6,0,ssfnum*D.YDIS*6,LocState.COMMITTED,null,false);
                ...
                if(ssfnum == 0){
                    loc = locSet(ssfnum*6+1,0,ssfnum*D.YDIS*6 + D.YDIS,LocState.COMMITTED,label,true);
                }
                else{
                    loc = locSet(ssfnum*6+1,0,ssfnum*D.YDIS*6 + D.YDIS,LocState.COMMITTED,label,false);
                }
                loclist = addLocation(loclist,loc);
                loc = locSet(ssfnum*6+2,D.XDIS,ssfnum*D.YDIS*6,LocState.COMMITTED,null,false);
                loclist = addLocation(loclist,loc);
                loc = locSet(ssfnum*6+3,D.XDIS*2,ssfnum*D.YDIS*6,LocState.COMMITTED,null,false);
                loclist = addLocation(loclist,loc);
                loc = locSet(ssfnum*6+4,D.XDIS*2,ssfnum*D.YDIS*6 + D.YDIS,LocState.COMMITTED,null,false);
                loclist = addLocation(loclist,loc);
                loc = locSet(ssfnum*6+5,D.XDIS*3,ssfnum*D.YDIS*6,LocState.COMMITTED,null,false);
                loclist = addLocation(loclist,loc);
                ssfnum++;
            }
        }
    ...
    Transition[] TransitionMake(Function.Func[] funclist,Location[] loc){
        ...
        for(int i=0;i<funclist.length;i++){
            if(funclist[i].type == Function.FuncType.SSF){
                ...
                trans = transSet(ssfnum*6,ssfnum*6+2,label,null);
                ...
                trans = transSet(ssfnum*6+5,ssfnum*6,label,nail);
                translist = addTransition(translist, trans);    
    ...
    Transition InitUpdateMake(Function.Func[] funclist,Function.Func ssf, int ssfnum){
		...
		trans = transSet(ssfnum * 6 + 1,ssfnum * 6 + 2,label,null);
        return trans;
    }
    ...
    Transition UpdateMake(Function.Func[] funclist,Function.Func ssf,int ssfnum){
		...
		trans = transSet(ssfnum * 6 + 2,ssfnum * 6 + 3,label,null);
        return trans;
    ...
    Transition[] BranchMake1(Function.Func[] funclist,Function.Func ssf,Transition[] translist,int ssfnum){
        ...
        for(int i=0;i < sf.sf.event.length;i++){
            ...
            if(sf.sf.event[i].eventval.equals("NoEvent")){
                ...
                trans = transSet(ssfnum*6 + 3,ssfnum*6 + 5,label,nail);
                ...
                trans = transSet(ssfnum*6 + 3,ssfnum*6 + 4,label,nail);
                aftertranslist = addTransition(aftertranslist,trans);
                eventnum++;
            }
            else{
    ...
    Transition[] BranchMake2(Function.Func[] funclist,Function.Func ssf,Transition[] translist,int ssfnum,Location[] loc){
        ...
        for(int i=0;i < f.f.change.length;i++){
            ...
            trans = transSet(ssfnum*6 + 4,locid,label,nail);
            aftertranslist = addTransition(aftertranslist,trans);
            num++;
        }
        return aftertranslist;
    ...
    Template TemplateRT(int locnum) {
		...
		Transition[] trans = new Transition[3];
		Label[] label;
		Nail[] nail;
		loc[0] = locSet(locnum+1,170,0,LocState.COMMITTED,null,true);
		loc[1] = locSet(locnum+2, 0, 0, LocState.COMMITTED,null,false);
        label = new Label[2];
        nail = new Nail[1];
        label[0] = labelSet(LabelKind.SYNCHRONISATION, 85, 45, "chk?");
        label[1] = labelSet(LabelKind.ASSIGNMENT,85,55,"Top_dt := 0,\nLow_dt := 0");
        nail[0]  = nailSet(85, 40);
        trans[0] = transSet(locnum+1, locnum+2, label, nail);

        label = new Label[2];
        nail = new Nail[1];
        label[0] = labelSet(LabelKind.SYNCHRONISATION, 70, -55, "trig!");
        label[1] = labelSet(LabelKind.GUARD, 85,-45,"compareER(Top_dt,Low_dt,sub(Top_Sampling,Low_Sampling,Top_Range,Low_Range,TOP),sub(Top_Sampling,Low_Sampling,Top_Range,Low_Range,LOW))");
		nail[0]  = nailSet(85, -40);
		trans[1] = transSet(locnum+2, locnum+1, label, nail);
     
        label = new Label[2];
        nail = new Nail[2];
        label[0] = labelSet(LabelKind.GUARD,-90,-80,"compareL(Top_dt,Low_dt,add(Top_Sampling,Low_Sampling,Top_Range,Low_Range,TOP),add(Top_Sampling,Low_Sampling,Top_Range,Low_Range,LOW))");
        label[1] = labelSet(LabelKind.ASSIGNMENT,-90,-70,"Top_Reg3 := Top_dt,\nLow_Reg3 := Low_dt,\nTop_dt := add(Top_Reg3,Low_Reg3,Top_Interval,Low_Interval,TOP),\nLow_dt := add(Top_Reg3,Low_Reg3,Top_Interval,Low_Interval,LOW)");
        nail[0] = nailSet(-85,-50);
        nail[1] = nailSet(-85,50);
        trans[2] = transSet(locnum+2,locnum+2,label,nail);
